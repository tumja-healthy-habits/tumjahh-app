...
export default function NewMosaicGridCopy({ mosaicRecord }: NewMosaicGridProps) {
    ...
    function generateSpiral(): (PhotosRecord & { latest: number } | undefined)[][] {
        ...
        photos.forEach((photo: PhotosRecord, i: number) => {
            grid[x + numRings][y + numRings] = (i < 8 ? { ...photo, latest: i } : photo) as PhotosRecord & { latest: number }
